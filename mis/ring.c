...
void recv_sharing(uint8_t *payload, uint8_t distance)
{
    ...
    //reset rounds
    mydata->nearest_neighbors[i].round=0;
    //mydata->round=0;
...
void send_sharing()
{
    ...
    if (mydata->now >= mydata->next_share_sending &&  mydata->message_sent == 1)
    {
        ...
        for(uint8_t i=0;i < mydata->num_neighbors;i++){
            mydata->nearest_neighbors[i].round=mydata->nearest_neighbors[i].round+1;
        }
...
void wakeup(){
	if(mydata->now >= mydata->next_wakeup){
        if (mydata->state == NOMIS) {
            for(uint8_t i=0;i < mydata->num_neighbors;i++){
        ...
        while (i < mydata->num_neighbors) {
            //for(uint8_t i=0;i<=mydata->num_neighbors;i++){
            if(mydata->nearest_neighbors[i].round > 5){
                printf("ID: %d :: Round #%d\n", mydata->nearest_neighbors[i].id, mydata->nearest_neighbors[i].round);
                mydata->num_neighbors--;
                memcpy(&mydata->nearest_neighbors[i], &mydata->nearest_neighbors[mydata->num_neighbors], sizeof(nearest_neighbor_t));
                //mydata->nearest_neighbors[i].state=NOMIS;
                //mydata->nearest_neighbors[i].random_value = 0;
                //mydata->nearest_neighbors[i].round=0;
                mydata->state=NOMIS;
            }
            else
...
void mis(){
    ...
    if(mydata->state==NOMIS && mydata->now >= mydata->next_mis){
        ...
        for (uint8_t i=0; i<mydata->num_neighbors; i++)
        {
            if(mydata->my_random_value > mydata->nearest_neighbors[i].random_value && mydata->nearest_neighbors[i].state==NOMIS)
            {
                flag=1;
                break;
            }
            if(mydata->nearest_neighbors[i].state==NOMIS)
            {
                counter++;
            }
...
void stabilization(){
    if(mydata->now >= mydata->next_stabilization){
        if(mydata->state == MIS) {
            for(uint8_t i=0;i < mydata->num_neighbors;i++){
                if(mydata->nearest_neighbors[i].state == MIS){
                    printf("ID:%d entered stabilization, NID: %d, Size: %d\n", mydata->my_id, mydata->nearest_neighbors[i].id, mydata->num_neighbors);
                    mydata->state=NOMIS;
                    break;
                }
            }
        } else if(mydata->state == PASSIVE) {
            uint8_t flag = 0;
            for(uint8_t i=0;i < mydata->num_neighbors;i++){
                if(mydata->nearest_neighbors[i].state == MIS){
                    flag = 1;
