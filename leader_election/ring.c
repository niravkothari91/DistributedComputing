...
static uint8_t final=0; //once found minimum will become 1 so rounds wont happen but if finds a smaller node again then becomes 0 and we will execute all rounds and then at the end of the round will again go back to 1 so as to stop further processing till finds a smaller one
//end
...
void prepare_message(uint8_t m, uint8_t receiver)
{
    ...
    mydata->msg.data[LEADER]=mydata->my_leader; //Tejas set leader field
...
//tejas added these so that when new node enters all rounds happen again and new node also knows the leader
...
void send_elected()
{
    ...
    {
        // Sending
        prepare_message(ELECTION, mydata->my_left); //Tejas changed my_leader to my_left
...
void setup() {
    ...
    mydata->green = 0,
    mydata->blue = 0,
    mydata->send_election = 1; //Tejas changed value from 0 to 1
    mydata->is_leader = 0;
